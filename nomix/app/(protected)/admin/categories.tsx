    ...
    >
      <View style={reportsModalStyles.overlay}>
        <View style={reportsModalStyles.container}>
          <LinearGradient
            colors={["rgba(0, 255, 136, 0.1)", "#1A1A2E"]}
...
const AdminCategories = () => {
  ...
  const [categoryReports, setCategoryReports] = useState<Report[]>([]);
  ...
  const fetchData = async () => {
    try {
      const [categoriesRes, reportsRes] = await Promise.all([
        getAllCategoriesAdmin(), // Uses admin endpoint
        getAllReports(),
      ]);
  ...
  const getReportsCountForCategory = (categoryId: string) => {
    return allReports.filter(
      (report) =>
        report.targetType === "category" &&
        (typeof report.targetId === "string"
          ? report.targetId === categoryId
          : report.targetId?._id === categoryId)
    ).length;
  };
  ...
  const filteredCategories = categories.filter((category) => {
    ...
    if (filter === "reported") {
      const reportsCount = getReportsCountForCategory(category._id || "");
  ...
  const sortedCategories =
    filter === "reported"
      ? [...filteredCategories].sort((a, b) => {
          const aReports = getReportsCountForCategory(a._id || "");
          const bReports = getReportsCountForCategory(b._id || "");
          return bReports - aReports;
        })
      : filteredCategories;

  const handleViewReports = async (category: Category) => {
    setSelectedCategory(category);
    setReportsModalVisible(true);

    try {
      // Fetch reports using the dedicated admin endpoint
      const result = await getReportsForCategory(category._id || "");
      setCategoryReports(result.data || []);
    } catch (error) {
  ...
  const handleDeleteCategory = (category: Category) => {
    showAlert(
      "confirm",
      "Delete Category",
      `Are you sure you want to delete "${category.name}"? This will affect all recipes using this category.`,
      async () => {
        setActionLoading(true);
        try {
          await adminDeleteCategory(category._id || ""); // Uses admin delete endpoint
  ...
  const renderCategoryItem = ({ item }: { item: Category }) => {
    const reportsCount = getReportsCountForCategory(item._id || "");
    const recipesCount = (item as any).recipes?.length || 0;
            ...
            <View style={styles.categoryDetails}>
              ...
              <View style={styles.statsRow}>
                <View style={styles.statItem}>
                  <Ionicons name="restaurant" size={12} color="#00FFFF" />
